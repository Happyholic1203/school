...
HfpDriver::HfpDriver() : TestDriver( "hfpage" )
{
	...
	{ 
		...
		if (status != OK) { 
			...
			cout << "Retrieved record, RID " << Rid.pageNo << ", "
				<< Rid.slotNo /*<< "value:" << *((int *)TmpBuf)*/ << endl;
		}
		if (size != sizeof(int)) 
...
{   const int BuffSize = 20;
	...
	cout << "\n  Test 3: Insert and Delete fixed-size records\n";
...
{   const int BuffSize = 20;
	...
	PageId tmpPID = 7; 
	Status status = OK;
	RID Rid, CurRid, ExList[Limit];
	int i, size, exchangeListSize=0; 	
	char TmpBuf[BuffSize]; 
	...
	cout << "\n  Test 7: Insert and Exchange fixed-size records\n";
	...
	{ status = hfp.getRecord(Rid, TmpBuf, size); 
		...
		CurRid=Rid; 
		if (i % 2 == 0 && i < Limit) { 
			ExList[exchangeListSize]=Rid; exchangeListSize++; 
		} 
		//if compaction is taking place, we shall see.
		status=hfp.nextRecord(CurRid, Rid);
		i++;
	...
	cout << "Exchange Records"<<endl;
	for (i=0; i<exchangeListSize; i++) 
	{ 
		if(i%4 == 2){
			RID tempRid = ExList[i+1];
			ExList[i+1] = ExList[i];
			ExList[i] = tempRid;
		}
		cout<<"Exchanging Rid("<<ExList[i].pageNo<<"," << ExList[i + 1].pageNo << "," << ExList[i].slotNo << "," << ExList[i + 1].slotNo<<").\n";
		status = hfp.exchangeRecord(ExList[i], ExList[i + 1]);
		i++;

		status = OK;
		if (status != OK) { 
			cout << "ERROR: Exchange of Rid(" << ExList[i].pageNo << "," << ExList[i].slotNo << ") and  Rid(" << ExList[i+1].pageNo << "," << ExList[i + 1].slotNo << ") failed\n";
		}
	}
	...
	while (status == OK)
	{ 
		cout << "Retrieving record " << Rid.pageNo << ", " << Rid.slotNo << "  ";
		status = hfp.getRecord(Rid, TmpBuf, size); 
		cout << "value:" <<  *((int *)TmpBuf) << endl;
		if (status != OK) { 
			cout << "ERROR: getting record.\n"; 
...
{   const int BuffSize = 50;
	...
	cout << "\n  Test 8: Insert and Exchange variable-length records\n";
	...
	{ status = hfp.returnRecord(Rid, temp, size); CurRid=Rid; 
		if (status != OK) { 
			...
			cout << "Retrieved record, RID " << Rid.pageNo << ", " << Rid.slotNo << "  ";
			char buf[100];
			strncpy(buf, temp, size);
			cout << "value:" <<  buf << endl;
		}
		if ((j%2 == 0)&&(j<Limit)) { ExList[exCNT]=Rid; exCNT++; } 
		//if compaction is taking place, we shall see.
		status=hfp.nextRecord(CurRid, Rid); j++; 
	} 
	if ( status != DONE ) 
	...
	{ 
		cout << "Retrieving record " << Rid.pageNo << ", " << Rid.slotNo << "  ";
		status = hfp.returnRecord(Rid, temp, size);
		char buf[100];
		strncpy(buf, temp, size);
		cout << "value:" <<  buf << endl;
		CurRid=Rid; 
